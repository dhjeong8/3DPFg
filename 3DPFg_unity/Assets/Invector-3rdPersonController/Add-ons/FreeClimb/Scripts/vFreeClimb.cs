...
namespace Invector.vCharacterController.vActions
{
    ...
    public class vFreeClimb : vMonoBehaviour
    {
        ...
        public string cameraState = "Strafing";
        ...
        protected virtual void Update()
        {
            if (CheckClimbCondictions())
            {
                ...
                ///get input state same as vThirdPersonInput
                if(TP_Input.cc.animator.GetCurrentAnimatorStateInfo(0).IsName(animatorStateHierarchy+".ClimbWall")){
                    //TP_Input.keyInputforAddon();
                    verticalKey = TP_Input.horizontalKey;
                    //verticalKey = TP_Input.horizontalKey;
                    doJump = TP_Input.doJump;
                    //Debug.Log("Jump: "+doJump);

                }
                input = new Vector3(horizontalKey,0, verticalKey);
                ClimbHandle();
                ClimbUpHandle();
                ClimbJumpHandle();
        ...
        {   
            ...
            if (dragInfo.canGo && !inClimbEnter && Physics.SphereCast(handTargetPosition + transform.forward * -TP_Input.cc._capsuleCollider.radius * 0.5f, TP_Input.cc._capsuleCollider.radius * 0.5f, transform.forward, out hit, climbEnterMaxDistance, draggableWall))
            {
                ...
                if (doJump && dragInfo.inDrag && input.magnitude == 0 && Time.time > (oldInput + 0.5f)){
                    ExitClimb();
                }
                
                else if (dragInfo.canGo && (doJump || TP_Input.cc.input.z != 0f || TP_Input.cc.input.x != 0f) && !dragInfo.inDrag && Time.time > (oldInput + 2f)){
        ...
        protected virtual void ClimbJumpHandle()
        {
            ...
            if (doJump && !inClimbJump && input.magnitude > 0 && !TP_Input.cc.animator.GetCurrentAnimatorStateInfo(0).IsName(animatorStateHierarchy + ".ClimbJump"))
            {
        ...
        {
            if (inClimbJump || TP_Input.enabled || !TP_Input.cc.animator || !dragInfo.inDrag){
                return;
            }
            ...
            if (!canMoveClimb && !inClimbUp && (climbUpConditions || ignoreInput))
            {
                ...
                if (!Physics.Linecast(startPoint, endPoint, obstacle))
                {
                    ...
                    {Debug.Log("1");
                        ...
                        if (true)//Physics.Linecast(thicknessPoint, climbPoint, out hit, TP_Input.cc.groundLayer))
                        {Debug.Log("2");
        ...
        protected void StaminaConsumption()
        {
            ...
            if (TP_Input.cc.currentStamina <= 0 && !(TP_Input.cc.animator.GetCurrentAnimatorStateInfo(0).IsName(animatorStateHierarchy + ".ClimbUpWall")))
            {
                ExitClimb();
            }
            else
        ...
        protected virtual void ExitClimb()
        {
            ...
            TP_Input.tpCamera.tempAngle = transform.forward;
