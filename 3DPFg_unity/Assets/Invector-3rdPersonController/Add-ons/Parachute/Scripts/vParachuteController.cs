...
public class vParachuteController : vMonoBehaviour
{
    ...
    public float parachuteStaminaCost = 5f;
    ...
    //float timer;
    //float waitingTime;
    public string animatorStateHierarchy = "Base Layer.Actions.Parachute";
    ...
    IEnumerator Start()
    {
        ...
        while((tpInput = GetComponentInParent<vThirdPersonInput>())==null);
    ...
    ///get input state same as vThirdPersonInput and add stamina consume when gliding
    protected virtual void Update(){
        if(tpInput==null)   return;
        if(tpInput.cc.animator.GetCurrentAnimatorStateInfo(0).IsName("Base Layer.Airborne.Falling.Falling")||
            tpInput.cc.animator.GetCurrentAnimatorStateInfo(0).IsName("Base Layer.Actions.Parachute.Parachute")){
                //tpInput.keyInputforAddon();
                horizontalKey = tpInput.horizontalKey;
                verticalKey = tpInput.verticalKey;
                doJump = tpInput.doJump;
    }

        if(tpInput.cc.animator.GetCurrentAnimatorStateInfo(0).IsName("Base Layer.Actions.Parachute.Parachute")){
            StaminaConsumption();
        }
    }
    ...
    ///change open parachute condition and delete exit parachute by key input(doJump did not update between open parachute and gliding)
    ...
    protected virtual void FixedUpdateParachute()
    {
        if (canMove)
        {
            var inputX = horizontalKey;
            var inputY = verticalKey;
    ...
    protected void StaminaConsumption()
    {
        ...
        if(parachuteStaminaCost == 0) return;
        if(tpInput.cc.currentStamina <= 0){
            ExitParachute();
        }else{
            tpInput.cc.ReduceStamina(parachuteStaminaCost, true);
